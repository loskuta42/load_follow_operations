...
def load_from_hdf5( h5_filename):
    ...
    return np.array(Zpn1),np.array(Zpn2),np.array(Zpn3),np.array(Zpn4),np.array(Zpn5),np.array(Zpn6)



if __name__ == "__main__":
    h5_filename = r"data_for_bgk.h5"
    Zpn1, Zpn2, Zpn3, Zpn4, Zpn5, Zpn6= load_from_hdf5(h5_filename)
    Zap_N = {}
    Zap_N[1] = Zpn1
    Zap_N[2] = Zpn2
    Zap_N[3] = Zpn3
    ...
    for key in Zap_N:
        ...
        plt.ylabel(r'$\frac{|| \Delta \vec{Zpn} ||}{ ||\vec{Zpn}|| }$, о.е.',fontdict=font)
        plt.xlabel("Номер состояния",fontdict=font)
        plt.legend()
        # plt.grid(True)
        #plt.savefig("main_state_error_Zpn{}.png".format(key))
        plt.show()
        ...
        plt.legend()
        #plt.grid(True)
        #plt.savefig("additional_state_error_Zpn{}.png".format(key))
        plt.show()
        ...
        par1 = host.twinx()
        host.set_xlabel(u"Размерность БГК Zpn")
        ...
        p3, = par1.plot(nredu_lst, Pn_vset1, "cp-", label=u"КИИ обучающий набор", markersize=5, linewidth=2)
        p4, = par1.plot(nredu_lst, Pn_vset2, "gd-", label=u"КИИ контрольный наборв", markersize=5, linewidth=2)
        #host.set_ylim(0, 0.07)
        host.legend(shadow=False, fancybox=True, loc=5)
        #host.grid(True)
        #host.set_title(u"оценка извлечения информаций")
        # plt.savefig(r"Er_mean_Pn_Zpn{}.png".format(key))
        plt.show()
        ...
        dic_errors_mean1 = {}
        dic_errors_mean2 = {}
        for key in range(len(errors_mean1)):
            dic_errors_mean1[key] = errors_mean1[key]

        for key in range(len(errors_mean2)):
            dic_errors_mean2[key] = errors_mean2[key]

        dic_Pn_vset1 = {}
        dic_Pn_vset2 = {}
        for key in range(len(Pn_vset1)):
            dic_Pn_vset1[key] = Pn_vset1[key]

        for key in range(len(Pn_vset2)):
            dic_Pn_vset2[key] = Pn_vset2[key]

        print(dic_errors_mean1)
        print(dic_errors_mean2)
        print("---------------------------------------------------")
        print(dic_Pn_vset1)
        print(dic_Pn_vset2)
